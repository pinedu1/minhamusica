...
describe('Classe Pausa', () => {
    ...
    describe('Funcionalidades Básicas', () => {
        it('deve lançar erro se nenhuma unidade de tempo for fornecida (falta de contexto rítmico)', () => {
            expect(() => {
                Pausa.create({ tempo: "1/4" });
            }).toThrow("Falha ao criar Pausa: 'unidadeTempo' (L:) não encontrado no contexto.");
        });
    ...
    describe('Helper estático Pausa.create() com JSON', () => {
        ...
        it('deve instanciar com strings usando "duracao" como referencia', () => {
            const pausa = Pausa.create({ tempo: "1/8", duracao: "1/4" });
            expect(pausa).toBeInstanceOf(Pausa);
        ...
        it('deve assumir denominador 1 se omitido na string de tempo ou duracao', () => {
            const pausa = Pausa.create({ tempo: "2", duracao: "1/4" });
