...
describe( 'Nota', () => {
    it( 'deve inicializar com valores padrão', () => {
        ...
        const nota = ObjectFactory.newNota( alturaMock, duracaoMock );
    ...
    it( 'deve falhar se altura não for NotaFrequencia', () => {
        const duracaoMock = new TempoDuracao(1,4);
        expect( () => ObjectFactory.newNota( {}, duracaoMock ) ).toThrowError( "A altura deve ser NotaFrequencia." );
    } );
    ...
    it( 'deve configurar opções personalizadas', () => {
        ...
        const nota = ObjectFactory.newNota( alturaMock, duracaoMock, options );
    ...
    it( 'deve falhar se graceNote for inválido', () => {
        const alturaMock = Object.create( NotaFrequencia.prototype );
        const duracaoMock = new TempoDuracao(1,4);
        expect( () => ObjectFactory.newNota( alturaMock, duracaoMock, { graceNote: true } ) ).toThrowError( "Nota: graceNote deve ser Array, null ou false." );
        expect( () => ObjectFactory.newNota( alturaMock, duracaoMock, { graceNote: {} } ) ).toThrowError( "Nota: graceNote deve ser Array, null ou false." );
    } );
	it( 'deve falhar se dedilhado for inválido', () => {
		const alturaMock = Object.create( NotaFrequencia.prototype );
		const duracaoMock = new TempoDuracao(1,4);
		expect( () => ObjectFactory.newNota( alturaMock, duracaoMock, { dedilhado: true } ) ).toThrowError( "Nota: dedilhado deve ser Array, null ou false." );
		expect( () => ObjectFactory.newNota( alturaMock, duracaoMock, { dedilhado: {} } ) ).toThrowError( "Nota: dedilhado deve ser Array, null ou false." );
		expect( () => ObjectFactory.newNota( alturaMock, duracaoMock, { dedilhado: [] } ) ).not.toThrowError();
	} );
